...
import 'package:zentri/absensi/services/absensi_repo.dart';
...
class AttendanceServices {
  ...
  Future<AbsensiResponse> getAbsensi(String token) async {
    // dapatkan response dari API
    final response = await getAbsensiAPI(token);
    final json = jsonDecode(response.body);

    if (response.statusCode == 200) {
      return AbsensiResponse.fromJson(json as Map<String, dynamic>);
    } else {
      throw Exception('Failed to load absensi');
    }
  }

  Future<AbsensiResponse> getAbsensiFiltered(
    String token,
    String tgl_start,
    String tgl_end,
  ) async {
